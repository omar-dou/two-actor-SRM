...
for index,row in inputs.iterrows():
  ...
  for t in range(t0,t5):
    ...
    #--A injects in both NH & SH, B injects in SH
    TSRMnh,TSRMsh,T0SRMnh,T0SRMsh=clim_nh_sh(TSRMnh,TSRMsh,T0SRMnh,T0SRMsh,f=f[t],gnh=gA,gsh=gA+gB,noise=noise)
    #--report T and RF into lists for plots
    T_SRM_nh.append(TSRMnh) ; T_SRM_sh.append(TSRMsh) ; gA_SRM_nh.append(gA) ; gB_SRM_sh.append(gB)
  ...
  plt.plot(gB_SRM_sh,label='SAI RF actor B',c='purple')
  plt.plot(T_noSRM_nh,label='NH dT w/o SRM',c='darkorange')
  plt.plot(T_noSRM_sh,label='SH dT w/o SRM',c='orange')
  plt.plot(T_SRM_nh,label='NH dT w SRM',c='darkgreen')
  plt.plot(T_SRM_sh,label='SH dT w SRM',c='green')
